...
void SystemCoreClockUpdate (void)
{
  ...
  switch (tmp)
  {
    ...
    case 0x0C:  /* HSI48 used as system clock */
      SystemCoreClock = 48000000u; // Fixed 48MHz
      break;
...
static void SetSysClock(void)
{
  ...
  { /* If HSE fails to start-up, the application will have wrong clock
    ...
    /*
     *
     * [14/05/07] Added by Segger.
     *
     * If no external crystal is connected
     * the interal 48MHz HSI is used
     *
     */
    /* Enable Prefetch Buffer and set Flash Latency */
    FLASH->ACR = FLASH_ACR_PRFTBE | FLASH_ACR_LATENCY;

    RCC->CR2 |= RCC_CR2_HSI48ON; // Turn on HSI48 oscillator
    /* Wait till  HSI48 oscillator is stable */
    while ((RCC->CR2 & (uint32_t)RCC_CR2_HSI48RDY) != (uint32_t)RCC_CR2_HSI48RDY)
    {
    }
    /* Select HSI48 oscillator as system clock source */
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_SW));
